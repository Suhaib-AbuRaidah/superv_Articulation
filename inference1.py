...
file_paths = "../Ditto/Articulated_object_simulation-main/data/Shape2Motion_gcn/cabinet/scenes/*.npz"
...
for f in glob.glob(file_paths):
    ...
    if count == 23:
        ...
        pc_start = data['pc_start_1']
        pc_target = data['pc_end_1']
        seg_mask_start_0 = data['pc_seg_start_0']
        seg_mask_start = data['pc_seg_start_1']
        seg_mask_target = data['pc_seg_end_1']
        joint_type = data['joint_type_1']
        screw_axis = data['screw_axis_1']
        screw_moment = data['screw_moment_1']
        state_start = data['state_start_1']
        state_target = data['state_end_1']
        ...
        seg_mask_start_1 = seg_mask_start.copy()
        ...
        data_tuple = (pc_start, pc_target, seg_mask_start, seg_mask_target,
                    center,scale,seg_mask_start_0, seg_mask_start_1,
...
pc_start, pc_target,seg_mask_start,_,center,scale,seg_mask_start_0,seg_mask_start_1,_,_,_,_,_,_,_,pc_start_unsampled,pc_target_unsampled = data_list[0]
...
colors[seg_mask_start_0==1] = np.array([1,0,0])
colors[seg_mask_start_1==1] = np.array([0,1,0])
...
tr1 = np.array([-0.14,0.2,0.36])
tr2 = np.array([-0.15,0.22,0.00])
sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.02)
sphere.paint_uniform_color([1, 0, 1]) 
sphere.translate(np.asarray(pivot_point_pred)+tr1,relative=False)

sphere2 = o3d.geometry.TriangleMesh.create_sphere(radius=0.02)
sphere2.paint_uniform_color([1, 0, 1]) 
sphere2.translate(np.asarray(pivot_point_pred)+tr2,relative=False)

arrow = create_axis_arrow(pivot_point_pred, joint_type_pred, length=0.55,tr=tr1)
arrow2 = create_axis_arrow(pivot_point_pred, joint_type_pred, length=0.55,tr=tr2)
