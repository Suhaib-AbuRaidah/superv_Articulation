...
def norm_attention(query, key, value, mask=None, dropout=None):
    """
    B*N*C
    """
    print("norm")
    d_k = query.size(-1)
    norm_q = query / query.norm(dim=-1, keepdim=True)
    norm_k = key / key.norm(dim=-1, keepdim=True)
    scores = torch.matmul(norm_q, norm_k.transpose(-2, -1).contiguous()) / math.sqrt(
        d_k
    )
    if mask is not None:
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x, self.score = attention(query, key, value, mask=mask, dropout=self.dropout)
...
class Attn(nn.Module):
    def __init__(self, args):
        self.residual = args.get("residual", False)
        self.norm_attn = args.get("norm_attn", False)
        super().__init__()

    def forward(self, src, tgt, return_score=False):
        """
        src: B*C*N_src
        dst: B*C*N_dst
        src_emb: B*C*N_src
        ...
        if self.norm_attn:
            src_emb, score = norm_attention(src, tgt, tgt)
        else:
            src_emb, score = attention(src, tgt, tgt)
        if self.residual:
            src_emb += src
        src_emb = src_emb.transpose(2, 1).contiguous()
        if return_score:
            return src_emb, score
        else:
            return src_emb
...
class TransformerAttn(nn.Module):
    def __init__(self, args):
        ...
        self.dropout = args["n_blocks"]
        self.ff_dims = args["ff_dims"]
        self.n_heads = args["n_heads"]
        self.residual = args.get("residual", False)
        self.return_score = args.get("return_score", False)
        c = copy.deepcopy
        attn = MultiHeadedAttention(self.n_heads, self.emb_dims)
        ff = PositionwiseFeedForward(self.emb_dims, self.ff_dims, self.dropout)
        self.model = EncoderDecoder(
            Encoder(
                EncoderLayer(self.emb_dims, c(attn), c(ff), self.dropout),
                self.N,
            ),
            Decoder(
    ...
    def forward(self, src, tgt, return_score=False):
        ...
        if self.return_score:
            score = self.model.decoder.layers[0].src_attn.score
            return src_embedding.transpose(2, 1).contiguous(), score
        else:
            return src_embedding.transpose(2, 1).contiguous()
    ...
    def forward(self, *input):
        ...
        if self.cycle:
            ...
            translation_ba = -torch.matmul(
                rotation_ba, translation_ab.unsqueeze(2)
            ).squeeze(2)
