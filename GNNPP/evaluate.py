...
def evaluation(model, data_dict, snap_pivot_to_contact=True):
    ...
    conn_acc  = (tp + tn) / (tp + tn + fp + fn + 1e-12)
    conn_prec = tp / (tp + fp+1e-12)
    conn_rec  = tp / (tp + fn+1e-12)
    ...
    joint_acc = (tp2 + tn2) / (tp2 + tn2 + fp2 + fn2 + 1e-12)
    prec2 = tp2 / (tp2 + fp2+1e-12)
    rec2  = tp2 / (tp2 + fn2+1e-12)
...
if __name__ == "__main__":
    ...
    dataset = PartsGraphDataset3(
        os.path.expanduser("~/superv_Articulation/data/Shape2Motion_gcn/cabinet_simp/train/scenes/*.npz"),
        device
    )
    ...
    checkpoint_path = os.path.expanduser(
        "~/superv_Articulation/pre_trained_models_gcnpp/2026-02-17 22_12_35_C/chkpt_17.pth"
    )
    ...
    with torch.no_grad():
        for index, data in enumerate(pbar_val):
            ...
            jt_tp += metrics.get("jt_tp", 0)
            jt_tn += metrics.get("jt_tn", 0)
            jt_fp += metrics.get("jt_fp", 0)
            jt_fn += metrics.get("jt_fn", 0)
    ...
    print(f"\n  Joint type distribution over evaluated edges: jt_tp={jt_tp}, jt_tn={jt_tn}, jt_fp={jt_fp}, jt_fn={jt_fn}")