    ...
    def forward(self, p, c_plane, **kwargs):
        if self.c_dim != 0:
            ...
            if self.concat_feat:
                ...
                if "grid" in plane_type:
                    c += self.sample_grid_feature(p, c_plane["grid"])
                if "corr_xz" in plane_type:
                    c += self.sample_plane_feature(p, c_plane["corr_xz"], plane="xz")
                if "corr_xy" in plane_type:
                    c += self.sample_plane_feature(p, c_plane["corr_xy"], plane="xy")
                if "corr_yz" in plane_type:
                    c += self.sample_plane_feature(p, c_plane["corr_yz"], plane="yz")
                c = c.transpose(1, 2)
