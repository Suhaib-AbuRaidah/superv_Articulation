...
class PartsGraphDataset3(Dataset):
    ...
    def __init__(self, file_paths, device, num_global_points=8192, num_part_points=1024, seed=97):
        ...
        for f in sorted(glob.glob(file_paths)):
            ...
            seg_mask_end = data["pc_seg_end"].item()
            pc_start_unsampled, seg_mask_start_unsampled = self.downsample_pc_masks(pc_start, seg_mask_start, pc_start.shape[0])
            pc_end_unsampled, seg_mask_end_unsampled = self.downsample_pc_masks(pc_end, seg_mask_end, pc_end.shape[0])   
